...
% Primal Bounds
bounds.phase(1).state.lower = [Stage2.Bounds.Alt(1), lonMin, latMin, Stage2.Bounds.v(1), Stage2.Bounds.gamma(1), Stage2.Bounds.zeta(1), Stage2.Bounds.mFuel(1), aoaMin, bankMin1];
bounds.phase(1).state.upper = [Stage2.Bounds.Alt(2), lonMax, latMax, Stage2.Bounds.v(2), Stage2.Bounds.gamma(2), Stage2.Bounds.zeta(2), Stage2.Bounds.mFuel(2), aoaMax, bankMax1];
...
% End States
bounds.phase(1).finalstate.lower = [40000, lonMin, latMin, Stage2.Bounds.v(1), Stage2.End.gammaOpt(1), Stage2.End.Zeta, Stage2.End.mFuel, aoaMin, 0];
bounds.phase(1).finalstate.upper = [40000, lonMax, latMax, Stage2.Bounds.v(2), Stage2.End.gammaOpt(2), Stage2.End.Zeta, Stage2.Initial.mFuel, aoaMax, 0];
...
% Initial Bounds
bounds.phase(2).initialstate.lower = [altMin, lonMin, latMin, speedMin, fpaMin, aziMin, mFuelMin, aoaMin, 0, throttleMin];
bounds.phase(2).initialstate.upper = [altMax, lonMax, latMax, speedMax, fpaMax, aziMax, mFuelMax, aoaMax, 0, throttleMax];

% State Bounds
bounds.phase(2).state.lower = [altMin, lonMin, latMin, speedMin, fpaMin, aziMin, mFuelMin, aoaMin, bankMin2, throttleMin];
bounds.phase(2).state.upper = [altMax, lonMax, latMax, speedMax, fpaMax, aziMax, mFuelMax, aoaMax, bankMax2, throttleMax];

% End State Bounds
bounds.phase(2).finalstate.lower = [altMin, lonMin, latMin, speedMin, deg2rad(-10), aziMin, Stage2.End.mFuel, aoaMin, bankMin2, throttleMin];
bounds.phase(2).finalstate.upper = [200, lonMax, latMax, speedMax, deg2rad(30), aziMax, Stage2.End.mFuel, aoaMax, bankMax2, throttleMax];
...
guess.phase(2).state   = [altGuess, lonGuess, latGuess, speedGuess, fpaGuess, aziGuess, mFuelGuess, aoaGuess, bankGuess,[0.0;0]];
