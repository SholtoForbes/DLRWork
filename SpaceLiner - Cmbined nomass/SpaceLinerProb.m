...
% lon0 = deg2rad(150.6097); % Rockhampton 
   ...
   lat0 = deg2rad(-22.532666); % Rockhampton
lon0 = deg2rad(150.809994); % Rockhampton
...
bounds.phase(1).state.lower = [0, -0.5, lat0-0.5, 0, -deg2rad(89), -2*pi, aoaMin, bankMin_ascent];
bounds.phase(1).state.upper = [200000, +0.5, lat0+0.5, 10000, deg2rad(89), 2*pi, aoaMax, bankMax_ascent];
end
if mission ==1 
...
% bounds.phase(1).initialstate.upper = [550,0.005, lat0+0.005, 60, deg2rad(88), 2*pi 1.37e+06, aoaMax, bankMax_ascent];


bounds.phase(1).initialstate.lower = [5900,-0.005, lat0-0.005, 250, deg2rad(63), deg2rad(-89), aoaMin, bankMin_ascent] ;
bounds.phase(1).initialstate.upper = [6100,0.005, lat0+0.005, 260, deg2rad(64), deg2rad(89), aoaMax, bankMax_ascent];

end

if mission == 4 
% bounds.phase(1).initialstate.lower = [500,0, lat0, 50, deg2rad(87), deg2rad(75), aoaMin, bankMin_ascent] ;
% bounds.phase(1).initialstate.upper = [550,0, lat0, 60, deg2rad(88), deg2rad(89), aoaMax, bankMax_ascent];
...
if mission ==3 
bounds.phase(1).initialstate.lower = [500,-0.005, lat0-0.005, 50, deg2rad(86), 0, 1.36e+06, aoaMin, bankMin_ascent] ;
...
% bounds.phase(8).state.upper(7) = 213213; %set max propellant of orbiter at separation 
...
% bounds.phase(8).finalstate.upper(7) = 213213;
...
bounds.phase(9).state.lower(8) = bankMin_descent;
bounds.phase(9).state.upper(8) = bankMax_descent;
...
bounds.phase(9).initialstate.lower(2) = lonMin;
bounds.phase(9).initialstate.lower(3) = latMin;
bounds.phase(9).initialstate.upper(2) = lonMax;
bounds.phase(9).initialstate.upper(3) = latMax;

% bounds.phase(9).state.upper(7) = 213213; %set max propellant at end of burn
% bounds.phase(9).initialstate.upper(7) = 213213;


if mission == 1|| mission == 4 || mission == 6

bounds.phase(9).finalstate.lower = [15000, lonF-lon0+2*pi-0.005, latF-0.005, 250,deg2rad(-40), -pi, aoaMin, bankMin_descent]; % Japan-Germany
bounds.phase(9).finalstate.upper = [20000, lonF-lon0+2*pi+0.005, latF+0.005, 350, deg2rad(40), pi, aoaMax, bankMax_descent];
...
bounds.phase(9).finalstate.lower = [15000, lonF-lon0-2*pi, latF, 250,deg2rad(-40), deg2rad(90), 0, aoaMin, bankMin_descent]; % Japan-Germany
bounds.phase(9).finalstate.upper = [20000, lonF-lon0-2*pi, latF, 350, deg2rad(40), deg2rad(270),213213, aoaMax, bankMax_descent];
end

if mission == 3 

bounds.phase(9).finalstate.lower = [15000, lonF-lon0-2*pi-0.005, latF-0.005, 250,deg2rad(-40), deg2rad(90), 0, aoaMin, bankMin_descent]; % FLorida-Aus
bounds.phase(9).finalstate.upper = [20000, lonF-lon0-2*pi+0.005, latF+0.005, 350, deg2rad(40), deg2rad(270),213213, aoaMax, bankMax_descent];
end

if mission == 5

bounds.phase(9).finalstate.lower = [15000, lonF-lon0+2*pi-0.005, latF-0.005, 250,deg2rad(-40), deg2rad(-90), 0, aoaMin, bankMin_descent]; 
bounds.phase(9).finalstate.upper = [20000, lonF-lon0+2*pi+0.005, latF+0.005, 350, deg2rad(40), deg2rad(90),213213, aoaMax, bankMax_descent];
end
...
bounds.eventgroup(9).lower = [ones(1,8)*10 10];
...
guess.phase(1).state(:,7)   = [1*pi/180; 1*pi/180];
...
if mission == 4 
guess.phase(2).state = guess.phase(1).state + [10000 10000; 0.0 0.0;.05 .05; 100 100; -deg2rad(0) -deg2rad(0);0 0;0 0;0 0]';
guess.phase(3).state = guess.phase(2).state+ [10000 10000; 0.0 0.0;.05 .05; 100 100; -deg2rad(0) -deg2rad(0);0 0;0 0;0 0]';
guess.phase(4).state = guess.phase(3).state+ [10000 10000; 0.0 0.0;.05 .05; 100 100; -deg2rad(0) -deg2rad(0);0 0;0 0;0 0]';
guess.phase(5).state = guess.phase(4).state+ [10000 10000; 0.0 0.0;.05 .05; 100 100; -deg2rad(0) -deg2rad(0);0 0;0 0;0 0]';
guess.phase(6).state = guess.phase(5).state+ [10000 10000; 0.0 0.0;.05 .05; 100 100; -deg2rad(0) -deg2rad(0);0 0;0 0;0 0]';
guess.phase(7).state = guess.phase(6).state+ [10000 10000; 0.0 0.0;.05 .05; 100 100; -deg2rad(0) -deg2rad(0);0 0;0 0;0 0]';
guess.phase(8).state = guess.phase(7).state+ [10000 10000; 0.0 0.0;.05 .05; 100 100; -deg2rad(0) -deg2rad(0);0 0;0 0;0 0]';
guess.phase(9).state = guess.phase(8).state+ [10000 -50000; 0.0 lonF-lon0+2*pi;1.5 latF; 1000 -500; -deg2rad(0) -deg2rad(60);0 -deg2rad(150);deg2rad(10) deg2rad(10);deg2rad(30) deg2rad(30)]';
end
...
guess.phase(9).time = guess.phase(8).time + 4000;
    ...
    guess.phase(2).time = guess.phase(1).time + 3; 
guess.phase(3).time = guess.phase(2).time + 3;
guess.phase(4).time = guess.phase(3).time + 3;
guess.phase(5).time = guess.phase(4).time + 3;
guess.phase(6).time = guess.phase(5).time + 3;
guess.phase(7).time = guess.phase(6).time + 3;
guess.phase(8).time = guess.phase(7).time + 20;
guess.phase(9).time = guess.phase(8).time + 4000;
end
...
if mission == 1 || mission == 4
setup.nlp.ipoptoptions.maxiterations = 500; % this can make most of the difference
else
...
[altdot,londot,latdot,gammadot(i),vdot(i),azidot(i), q(i), M(i), Fd(i), rho,L(i),mFuel(i),T(i),Isp1,Isp2,m(i),heating_rate(i),total_acceleration(i),Cl(i),Cd(i)] = SpaceLinerVehicleModel(time(i),phase_temp,throttle(i),auxdata,stage(i));
